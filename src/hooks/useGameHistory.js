...
export function useGameHistory() {
  ...
  const allGames = useMemo(() => MOCK_GAMES, []);

  const filtered = useMemo(() => {
    let list = allGames;
    if (filter !== "all") list = list.filter((g) => g.outcome === filter);
    if (search) list = list.filter((g) => g.txId.toLowerCase().includes(search.toLowerCase()));
    return list;
  ...
  return {
    games, loading, total: filtered.length,
    page, setPage, totalPages,
    filter, setFilter,
    search, setSearch,
    stats, perPage: PER_PAGE,
  };
}
