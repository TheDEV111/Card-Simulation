...
const ACHIEVEMENTS = [
  ...
  { id: "streak_3",     title: "Hot Streak",         desc: "Win 3 games in a row",              icon: "🌶️", target: 3,   stat: "streak" },
  { id: "streak_5",     title: "Unstoppable",        desc: "Win 5 games in a row",              icon: "⚡", target: 5,   stat: "streak" },
];

export function useRewards(stats) {
  return useMemo(() => {
    if (!stats) return { achievements: [], claimed: 0 };
    ...
    return {
      achievements,
      claimed:   achievements.filter((a) => a.unlocked).length,
